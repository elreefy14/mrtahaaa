...
class CommentSheetController extends BaseController {
  ...
  // ------- Helper methods to add/update comments from UI (text, voice, gif) --------
  void onAddComment(Comment comment, bool isReplyComment) {
    if (isReplyComment) {
      RxList<Comment> items = getReplyCommentsList.putIfAbsent(
          comment.commentId?.toInt() ?? -1, () => <Comment>[].obs);
      items.insert(0, comment);
  ...
  void updateCommentList(int id, int count) {
    if (id != -1) {
      ...
      if (_comment != null) {
        ...
        commentsList[commentsList.indexWhere((e) => e.id == id)] = _comment;
      }
    }
  }
