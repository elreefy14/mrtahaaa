...
import 'package:bubbly/common/service/api/search_service.dart';
import 'package:bubbly/common/service/api/user_service.dart';
import 'package:bubbly/common/service/url_extractor/metadata_extract_base.dart';
import 'package:bubbly/common/service/url_extractor/parsers/base_parser.dart';
...
class CommentHelper {
  ...
  // indicates bottom recording container expansion
  late final AnimationController audioAnimationController;
  Animation<double>? audioWidthAnimation;
  RxBool isRecording = false.obs;
  // call this from UI after helper is created to init animation controller with vsync
  void initRecorderAnimation(TickerProvider vsync) {
    ...
    audioWidthAnimation = Tween<double>(begin: 0, end: maxWidth).animate(
        CurvedAnimation(parent: audioAnimationController, curve: Curves.easeInOut));
  }

  Future<void> startRecording() async {
    bool granted = await recorderController.checkPermission();
    if (!granted) {
      Get.bottomSheet(
          ConfirmationSheet(
  ...
  Future<void> sendRecordedAudio(Post reel, Function(Comment, bool) onUpdate) async {
    audioAnimationController.reverse();
    isRecording.value = false;
    try {
      String? path = await recorderController.stop();
      if (path == null) {
        Loggers.error('Recorded audio path not found');
        return;
      }
      // Directly upload the audio file as the `comment` field in a multipart
      // request because the backend expects the raw file when `type = 2`.
      Comment? comment = await PostService.instance.addVoiceComment(
        postId: reel.id?.toInt() ?? -1,
        file: XFile(path),
      );
      if (comment != null) {
      ...
      Loggers.error('Error sending voice comment: $e');
    }
  }
  ...
  Future<void> onCommentPost({
    ...
    Comment? comment;
    if (commentType == CommentType.voice) {
      // server already stores url in comment param after upload in UI layer
      comment = await handleAddComment(description, commentType, reel,
