...
import 'package:bubbly/common/service/api/add_post_story_service.dart';
import 'package:bubbly/common/service/api/common_service.dart';
import 'package:bubbly/common/service/api/post_service.dart';
import 'package:bubbly/common/service/sight_engin/sight_engine_service.dart';
import 'package:bubbly/common/service/utils/params.dart';
import 'package:bubbly/languages/languages_keys.dart';
import 'package:bubbly/model/general/file_path_model.dart';
import 'package:bubbly/model/general/location_place_model.dart';
import 'package:bubbly/model/general/place_detail.dart';
import 'package:bubbly/model/general/settings_model.dart';
import 'package:bubbly/model/post_story/music/music_model.dart';
...
import 'package:bubbly/screen/color_filter_screen/widget/color_filtered.dart';
...
import 'package:bubbly/screen/selected_music_sheet/selected_music_sheet_controller.dart';
...
class CreateFeedScreenController extends BaseController {
  ...
  // متغير حالة الرفع - يتحكم في UI الزر
  ...
  /// دالة الرفع الرئيسية مع إدارة حالة loading
  void handleUpload() async {
    ...
    if (isUploading.value) {
      Loggers.warning('Upload already in progress. Ignoring duplicate request...');
      return;
    }

    // التحقق من وجود محتوى للرفع
    if (_shouldAbortUpload()) {
      Loggers.warning('Nothing to upload. Aborting...');
      return;
    }

    // بدء حالة الرفع - تعطيل الزر وعرض loading
    ...
    try {
      ...
      Loggers.info('Post Data prepared: $postParams');
      ...
      Loggers.error('Error during upload process: $e');
      failedResponseSnackBar(message: '$e');
    } finally {
      // إنهاء حالة الرفع دائماً - إعادة تفعيل الزر
      isUploading.value = false;
      Loggers.info('Upload process completed. Button re-enabled.');
    }
  }
  ...
  /// دالة معالجة الأخطاء مع إنهاء حالة الرفع
  Future<PostModel?> failedResponseSnackBar({String? message}) async {
    _lastUploadType = UploadType.error;
    updateUploadingProgress(progress: 100);

    // إنهاء حالة الرفع عند الفشل - إعادة تفعيل الزر
    isUploading.value = false;
    Loggers.error('Upload failed: $message');
