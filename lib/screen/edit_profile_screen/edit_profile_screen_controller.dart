...
import 'package:bubbly/screen/edit_profile_screen/widget/phone_codes_screen_controller.dart';
...
class EditProfileScreenController extends BaseController {
  ...
  void onSaveTap() async {
    ...
    if (usernameController.text.trim().length < 4) {
      return showSnackBar(LKey.usernameEmpty.tr); // Add this key to your language files
    }
  ...
  void checkUsernameAvailability(String value) {
    ...
    // If username is empty, consider it valid (for now)
    if (username.isEmpty) {
      isValidUserName.value = true;
      return;
    }
