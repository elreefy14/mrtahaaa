...
class StoryView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        children: [
          // Your Story Section - استخدم GetBuilder بدلاً من Obx
          GetBuilder<FeedScreenController>(
            id: 'user_info',
            builder: (controller) {
              return _buildYourStory(context, controller);
            },
          ),
          // Other Stories Section - استخدم GetBuilder بدلاً من Obx
          Expanded(
            child: GetBuilder<FeedScreenController>(
              id: 'stories_list',
              builder: (controller) {
                return _buildOtherStories(context, controller);
              },
            ),
          ),
        ],
  ...
  Widget _buildYourStory(BuildContext context, FeedScreenController controller) {
  ...
  Widget _buildOtherStories(BuildContext context, FeedScreenController controller) {
    List<User> stories = controller.stories;

    if (stories.isEmpty) {
      return const SizedBox(); // إرجاع widget فارغ إذا لم توجد قصص
    }
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        return _buildOtherStory(context, stories, index);
      },
    );
  }

  Widget _buildOtherStory(BuildContext context, List<User> users, int index) {
      ...
      child: Column(
        ...
        children: [
          GradientBorder(
            ...
            radius: 90,
            onPressed: () {
              // تمرير الـ controller بشكل مباشر
              final feedController = Get.find<FeedScreenController>();
              feedController.onWatchStory(users, index, 'other_story');
            },
            child: Padding(
