...
import 'package:bubbly/common/widget/dominant_color.dart';
import 'package:bubbly/languages/languages_keys.dart';
import 'package:bubbly/model/general/status_model.dart';
import 'package:url_launcher/url_launcher.dart';

extension StringExtention on String {
  /// Returns an absolute URL for media assets stored on the server.
  ///
  /// * If the current string already begins with `http`, it is assumed to be
  ///   an absolute URL and returned unchanged.
  /// * For S3 paths (like voice comments), constructs proper S3 URLs
  /// * Otherwise the global `itemBaseUrl` from settings is **prepended**. Any
  ///   leading `/` in the original path is removed to avoid `//` duplicates.
  String addBaseURL() {
    ...
    // Handle S3 voice comment paths specifically
    if (contains('suplleexx/comments/') || startsWith('/suplleexx/')) {
      const String s3BaseUrl = 'https://s3.eu-north-1.amazonaws.com';
      String cleanPath = startsWith('/') ? substring(1) : this;
      return '$s3BaseUrl/$cleanPath';
    }

    String base = SessionManager.instance.getSettings()?.itemBaseUrl ?? '';
    if (base.isEmpty) return this;

    // Prevent double slashes when path already contains a leading slash
    String path = startsWith('/') ? substring(1) : this;
