...
import 'package:bubbly/model/user_model/follower_model.dart';
import 'package:bubbly/model/user_model/following_model.dart';
import 'package:bubbly/model/user_model/links_model.dart';
import 'package:bubbly/model/user_model/user_model.dart';
import 'package:bubbly/model/user_model/users_model.dart';
import 'package:bubbly/screen/edit_profile_screen/widget/add_edit_link_sheet.dart';
...
class UserService {
  ...
  Future<User?> logInUser({
    ...
    final requestBody = {
      ...
      if (phoneNumber != null && phoneNumber.isNotEmpty) Params.userMobileNo: phoneNumber,
      if (mobileCountryCode != null) Params.mobileCountryCode: mobileCountryCode,
    };

    // طباعة URL
    print('🌐 Request URL: ${WebService.user.loginInUser}');
    ...
    final headers = {
      ...
      // أضف أي headers إضافية هنا
      if (SessionManager.instance.getAuthToken()?.isNotEmpty == true)
        'Authorization': 'Bearer ${SessionManager.instance.getAuthToken()}',
    };
    ...
    try {
      ...
      if (model.status == true) {
        // Persist immediately to ensure follow-up calls (like updateUserDetails) use the right auth
